...
class BasePlugin:
    ...
    def pollnodes(self):
        BatteryNodes = {}
        APIjson = DomoticzAPI("type=devices&rid={}".format(Parameters["Mode5"]))
        try:
        ...
        for node in nodes:  # loop all nodes received from domoticz
            Domoticz.Debug(
                "Node {} {} has battery level of {}%".format(node["ID"], node["Name"], node["BatteryLevel"]))
            ...
            if node["battery"] != 255:  # battery level = 255 if not a battery device
                if not (node["nodeID"] in Devices):
                    Domoticz.Device(Name=node["Name"] if node["Name"] != "" else "Node {}".format(node["ID"]),
                                    Unit=node["ID"], TypeName="Custom",
                                    Options={"Custom": "1;%"}).Create()
                BatteryNodes[node["ID"]] = node["BatteryLevel"]
